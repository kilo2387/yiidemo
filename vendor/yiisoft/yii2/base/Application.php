...
abstract class Application extends Module
{
     ...
     */
    const EVENT_BEFORE_REQUEST = 'beforeRequest';   //接收请求前
    /**
     * @event Event an event raised after the application successfully handles a request (before the response is sent out).
     */
    const EVENT_AFTER_REQUEST = 'afterRequest';     //接收请求后
    /**
     * Application state used by [[state]]: application just started.
     */
    const STATE_BEGIN = 0;      //应用开始状态
    /**
     * Application state used by [[state]]: application is initializing.
     */
    const STATE_INIT = 1;       //应用初始化
    /**
     * Application state used by [[state]]: application is triggering [[EVENT_BEFORE_REQUEST]].
     */
    const STATE_BEFORE_REQUEST = 2;     //触发 EVENT_BEFORE_REQUEST
    /**
     * Application state used by [[state]]: application is handling the request.
     */
    const STATE_HANDLING_REQUEST = 3;       //加载 request
    /**
     * Application state used by [[state]]: application is triggering [[EVENT_AFTER_REQUEST]]..
     */
    const STATE_AFTER_REQUEST = 4;  //触发 EVENT_AFTER_REQUEST
    /**
    ...
    {
        Yii::$app = $this;  //把当前对象 yii\base\Application对象给Yii::$app

        //Yii::$app->loadedModules[get_class($instance)] = $instance; = Yii::$app->loadModules['yii\web\Application'] = $this
        static::setInstance($this);
//var_dump(Yii::$app->loadedModules);die('loadedModules');
        $this->state = self::STATE_BEGIN;   // 应用开始

        $this->preInit($config);    //初始化配置

        $this->registerErrorHandler($config);   //注册异常机制

        Component::__construct($config);    //再次调用 yii\base\Object 的__construct()方法
    }
...
//        var_dump($this->getBasePath());die('sre');
    }
